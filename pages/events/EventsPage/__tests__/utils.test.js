...
describe('sortPastEvents', () => {
  ...
  const date2 = faker.date.past(undefined, date1);
  const date3 = faker.date.past(undefined, date2);

  it('when all events have no end date, then the events are sorted in reverse chronological order (today > last year) by start date', () => {
    ...
    const actual = sortPastEvents([event2, event3, event1]);
